...
export function ProduitOrderPanel({ produit, variante }: ProduitOrderPanelProps) {
  ...
  const canOrder = !!variante && variante.quantiteStock > 0;
  const maxQte = variante ? Math.min(variante.quantiteStock, 10) : 1;
  ...
  const handleAddToCart = () => {
    if (!variante) return;
    addToCart({ produit, variante, quantite });
    setCartOpen(true);
  };
  ...
  return (
    ...
    >
      ...
      {variante && variante.quantiteStock === 0 && (
      ...
      {canOrder && (
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-3 rounded-xl border px-3 py-2" style={{ borderColor: "var(--v-border)" }}>
            ...
            <span className="w-6 text-center font-[var(--font-mono)] font-black text-sm" style={{ color: "var(--v-text)" }}>
              {quantite}
            </span>
            <button
      ...
      {canOrder && (
        <div className="flex items-center justify-between border-t pt-4" style={{ borderColor: "var(--v-border)" }}>
          ...
          <span className="font-[var(--font-mono)] text-xl font-black" style={{ color: "var(--v-lime)" }}>
